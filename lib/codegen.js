...
var getViewForSwagger2 = function(opts, type) {
    ...
    var isObject = function(definition) {
        return definition.type === 'object';
    };

    var isPartOf = function(definition) {
        return definition.hasOwnProperty('allOf') && _.isArray(definition.allOf);
    };

    var getParentTypeName = function(definition) {
        if (!isPartOf(definition)) {
            return null;
        }

        var def = null;
        _.forEach(definition.allOf, function(definitionPart) {
            if (_.isString(definitionPart.$ref)) {
                def = definitionPart.$ref.split('/')[2];
            }
        });
        return def;
    };

    var getProperties = function(definition) {
        if (!isObject(definition)) {
            return null;
        }

        var props = [];
        _.forEach(definition.properties, function(propDef, propName) {
            var isDate = propDef.type === 'string' && ['date', 'date-time'].indexOf(propDef.format) >= 0;
            var isArray = propDef.type === 'array';
            var isObject = propDef.type === 'object';
            props.push({
                name: propName,
                type: propDef.type,
                ...
                isArray: isArray,
                isObject: isObject,
                isPrimitive: !isDate && !isArray && !isObject,
    ...
    var getComposedObjectProperties = function(composedType) {
        if (!isPartOf(composedType)) {
            return null;
        }

        var properties = [];
        _.forEach(composedType.allOf, function(definitionPart) {
            if (!definitionPart.hasOwnProperty('$ref')) {
                properties = properties.concat(getProperties(definitionPart));
            }
        });
        return properties;
    };

    var getPropertiesNames = function(properties) {
    ...
    _.forEach(swagger.definitions, function(definition, definitionName) {
        if (!isObject(definition) && !isPartOf(definition)) {
            return;
        }

        var parentType = getParentTypeName(definition);
        var isExtending = parentType !== null;

        var properties;
        if (isExtending) {
            properties = getComposedObjectProperties(definition);
        } else {
            properties = getProperties(definition);
        }
        propertiesMap.set(definitionName, properties);
        ...
        objectsMap.set(definitionName, object);
        data.objects.push(object);
    });
    _.forEach(data.objects, function(object) {
        var parentProperties = [];
        var objectIterator = object;
        while (objectIterator.isExtending) {
            objectIterator = objectsMap.get(objectIterator.extends);
            parentProperties = propertiesMap.get(objectIterator.objectName).concat(parentProperties);
        }
        object.parentPropertiesAsParameters = getPropertiesNames(parentProperties).join(', ');
        object.propertiesAsParameters = getPropertiesNames(parentProperties.concat(object.properties)).join(', ');
    });
    return data;
};
...
var getCode = function(opts, type) {
    ...
    if (type === 'custom') {
        if (!_.isObject(opts.template) || !_.isString(opts.template.class) || !_.isString(opts.template.method) || !_.isString(opts.template.object) || !_.isString(opts.template.request)) {
            throw new Error('Unprovided custom template. Please use the following template: template: { class: "...", method: "...", object: "...", request: "..." }');
        }
    } else {
        ...
        opts.template.object = opts.template.object || fs.readFileSync(templates + 'object.mustache', 'utf-8');
